...
import org.apache.poi.hwpf.converter.PublicHtmlDocumentFacade;
...
public class WordToHtmlConverter extends AbstractWordConverter {
	...
	private final PublicHtmlDocumentFacade htmlDocumentFacade;
	...
	public WordToHtmlConverter(Document document) {
		this.htmlDocumentFacade = new PublicHtmlDocumentFacade(document);
	}

	public WordToHtmlConverter(PublicHtmlDocumentFacade htmlDocumentFacade) {
		this.htmlDocumentFacade = htmlDocumentFacade;
	}

	static Document process(File docFile) throws Exception {
		final HWPFDocumentCore wordDocument = PublicWordToHtmlUtils
				.loadDoc(docFile);
	...
	protected void outputCharacters(Element pElement,
		...
		if (PublicWordToHtmlUtils.isNotEmpty(triplet.fontName)
				&& !PublicWordToHtmlUtils.equals(triplet.fontName,
		...
		PublicWordToHtmlUtils.addCharactersProperties(characterRun, style);
		if (style.length() != 0)
	...
	protected void processDocumentInformation(
		...
		if (PublicWordToHtmlUtils.isNotEmpty(summaryInformation.getTitle()))
			htmlDocumentFacade.setTitle(summaryInformation.getTitle());

		if (PublicWordToHtmlUtils.isNotEmpty(summaryInformation.getAuthor()))
			htmlDocumentFacade.addAuthor(summaryInformation.getAuthor());

		if (PublicWordToHtmlUtils.isNotEmpty(summaryInformation.getKeywords()))
			htmlDocumentFacade.addKeywords(summaryInformation.getKeywords());

		if (PublicWordToHtmlUtils.isNotEmpty(summaryInformation.getComments()))
	...
	protected void processParagraph(HWPFDocumentCore hwpfDocument,
		...
		StringBuilder style = new StringBuilder();
		PublicWordToHtmlUtils.addParagraphProperties(paragraph, style);
...

		{
			...
			if (characterRun != null) {
				...
				PublicWordToHtmlUtils.addFontFamily(pFontName, style);
				PublicWordToHtmlUtils.addFontSize(pFontSize, style);
			} else {
				pFontSize = -1;
				pFontName = PublicWordToHtmlUtils.EMPTY;
			}
			blocksProperies.push(new BlockProperies(pFontName, pFontSize));
		}
		try {
			if (PublicWordToHtmlUtils.isNotEmpty(bulletText)) {
				if (bulletText.endsWith("\t")) {
					...
					Element span = htmlDocumentFacade.getDocument()
							.createElement("span");
					htmlDocumentFacade.addStyleClass(span, "s",
							"display: inline-block; text-indent: 0; min-width: "
									+ (spanMinWidth / TWIPS_PER_INCH) + "in;");
					pElement.appendChild(span);
		...
		PublicWordToHtmlUtils.compactSpans(pElement);
		return;
	}
	...
	protected void processTable(HWPFDocumentCore hwpfDocument, Element flow,
		...
		final int[] tableCellEdges = PublicWordToHtmlUtils
		...
		for (int r = 0; r < tableRows; r++) {
			...
			PublicWordToHtmlUtils
					.addTableRowProperties(tableRow, tableRowStyle);
			...
			for (int c = 0; c < rowCells; c++) {
				...
				PublicWordToHtmlUtils.addTableCellProperties(tableRow,
						tableCell, r == 0, r == tableRows - 1, c == 0,
						c == rowCells - 1, tableCellStyle);
				...
				if (tableCellStyle.length() > 0)
					htmlDocumentFacade.addStyleClass(tableCellElement,
							tableCellElement.getTagName(), tableCellStyle
									.toString());
